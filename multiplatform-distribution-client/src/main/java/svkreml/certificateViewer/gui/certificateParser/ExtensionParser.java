...
public class ExtensionParser {
    ...
    public static CertificateModel.CertificateDetail parseExtension(Localization localization, Extension extension) throws ParseException {
...
/*            case "1.3.6.1.4.1.11129.2.4.2":
    ...
    private static void parseEnhancedKeyUsage(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
...
/*
    private static void parseExtendedValidationCertificate(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        parsedAsn1Ext.append("");
    }
*/

    private static void parseSubjectAlternativeName(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        GeneralNames instance = GeneralNames.getInstance(extension.getParsedValue());
        GeneralName[] names = instance.getNames();
        for (GeneralName name : names) {
            parsedAsn1Ext.append(convertGeneralNameTag(name.getTagNo())).append(" : ").append(name.getName().toString()).append('\n');
        }
    ...
    private static void parseAuthorityInformationAccess(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        ...
        for (AccessDescription accessDescription : instance.getAccessDescriptions()) {
            parsedAsn1Ext.append(localization.convertOidToString(accessDescription.getAccessMethod(), "")).
                    append("(").append(accessDescription.getAccessMethod().getId()).append(")").append(" : ").
                    append(DERIA5String.getInstance(accessDescription.getAccessLocation().getName()).getString()).append('\n');
        }
    }


    private static void parseBasicConstraints(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        ...
        parsedAsn1Ext.append(localization.EXTENSIONS_PATH_LEN_CONSTRAINT).append(" ").append(instance.getPathLenConstraint()).append("\n");
    }

    private static void parseCertificatePolices(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        ...
        for (PolicyInformation policyInformation : instance.getPolicyInformation()) {
            ...
            if (policyQualifiers != null)
                parsedAsn1Ext.append(policyQualifiers.toString());
            parsedAsn1Ext.append('\n');
        }
    }

    private static void parseCertificateRevocationList(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        CRLDistPoint crlDistPoint = CRLDistPoint.getInstance(extension.getParsedValue());
        parsedAsn1Ext.append(crlDistPoint.toString().replaceAll("([\\n\\r\\]]){4,}","\n]]\n")); //FIXME распарсить все варианты
   /*     //List<String> crlUrls = new ArrayList<String>();
    ...
    private static void parsePrivateKeyUsagePeriod(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) throws ParseException {
        PrivateKeyUsagePeriod instance = PrivateKeyUsagePeriod.getInstance(extension.getParsedValue());
        parsedAsn1Ext.append(String.format("%-15s %s", localization.EXTENSIONS_PRIVATE_KEY_VALID_FROM, localization.formatDate(instance.getNotBefore().getDate()))).append("\n");
        parsedAsn1Ext.append(String.format("%-15s %s", localization.EXTENSIONS_PRIVATE_KEY_VALID_TO, localization.formatDate(instance.getNotAfter().getDate()))).append("\n");
    }

    private static void parseExtendedKeyUsage(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        ExtendedKeyUsage instance = ExtendedKeyUsage.getInstance(extension.getParsedValue());
        for (KeyPurposeId usage : instance.getUsages()) {
            parsedAsn1Ext.append(localization.convertOidToString(usage.toOID())).append("(").append(usage.toOID()).append(")").append("\n");
        }
    }

    private static void parseSubjectSignatureTool(Extension extension, StringBuilder parsedAsn1Ext) {
        parsedAsn1Ext.append("Наименовании средства ЭП Субъекта: ").append(DERUTF8String.getInstance(DEROctetString.getInstance(extension.getExtnValue()).getOctets()).getString());
    }

    private static void parseIssuerSignatureTool(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
    ...
    private static void parseSubjectKeyIdentifier(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
    ...
    private static void parseKeyUsage(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
        ...
        if (keyUsage.hasUsages(KeyUsage.cRLSign))
            parsedAsn1Ext.append("Автономное подписание списка отзыва (CRL), Подписывание списка отзыва (CRL)").append("\n");
    ...
    private static void parseAuthorityKeyIdentifier(Localization localization, Extension extension, StringBuilder parsedAsn1Ext) {
 ...
 /*       RDN[] rdNsReverse = new RDN[rdNs.length];
        ...
        for (RDN rdN : rdNs) {
            ...
            String[] names = X500Name.getDefaultStyle().oidToAttrNames(new ASN1ObjectIdentifier(rdN.getFirst().getType().toString()));
            map.put(names.length > 0 ? names[0].toUpperCase() : rdN.getFirst().getType().toString(), rdN.getFirst().getValue().toString());
        }
        return map;
    }
}
